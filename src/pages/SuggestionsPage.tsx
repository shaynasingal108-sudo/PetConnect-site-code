...
export default function SuggestionsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'quiz' | 'ask' | 'search'>('quiz');
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'quiz' | 'ask' | 'search')}>
        <TabsList className="grid w-full grid-cols-3">
          <TabsTrigger value="quiz" className="flex items-center gap-1 text-xs sm:text-sm">
            <Brain className="h-4 w-4" /> Quiz
          </TabsTrigger>
          <TabsTrigger value="ask" className="flex items-center gap-1 text-xs sm:text-sm">
            <Lightbulb className="h-4 w-4" /> Ask AI
          </TabsTrigger>
        ...
        <TabsContent value="ask" className="space-y-4 mt-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              <Textarea
                placeholder="E.g., How do I train my puppy to stop biting? What food is best for senior cats? How often should I groom my golden retriever?"
                value={question}
